...
public class BlockRegistry {
	...
	public static final Block PATH_FINDING_END_NODE_BLOCK = registerBlock("path_finding_end_node_block", new PathFindingEndNodeBlock(Block.Settings.create().mapColor(MapColor.LIGHT_GRAY).nonOpaque().requiresTool().strength(-1.0f, 3600000.0f).dropsNothing()), ItemGroups.OPERATOR);
	public static final Block VISIBLE_PATH_FINDING_END_NODE_BLOCK = Registry.register(Registries.BLOCK, MobBrainAdditions.identifier("visible_path_finding_end_node_block"), new Block(Block.Settings.create().mapColor(MapColor.LIGHT_GRAY).nonOpaque().requiresTool().strength(-1.0f, 3600000.0f).dropsNothing()));
