...
/**
 * This sensor checks if the entity is in range of its home position. If the distance is larger than allowed, the {@link MemoryModuleTypeRegistry#IS_NEAR_HOME}, {@link MemoryModuleType#ATTACK_TARGET} and {@link MemoryModuleType#WALK_TARGET} memories are cleared.
...
public class IsNearHomeSensor<E extends MobEntity> extends ExtendedSensor<E> { // Extend PredicateSensor so we can use the builtin predicate to check for lava
	 ...
	 *
	 * @param maxDistanceFromHome max distance from the path, in blocks
	 * @return this
	 */
	...
	protected void sense(ServerWorld level, E entity) {
		...
		if (entity.getBlockPos().getManhattanDistance(homePos.pos()) <= this.maxDistanceFromHome) {
